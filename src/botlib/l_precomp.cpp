...
int PC_EvaluateTokens( source_t *source, token_t *tokens, signed long int *intvalue,
	...
	while ( !error && firstoperator )
	{
		...
		for ( o = firstoperator; o->next; o = o->next )
		{
			...
			if ( o->m_operator != P_LOGIC_NOT
				 && o->m_operator != P_BIN_NOT ) {
				v = v->next;
			}
...
#ifdef DEBUG_EVAL
		if ( integer ) {
			Log_Write( "operator %s, value1 = %d", PunctuationFromNum( source->scriptstack, o->m_operator ), v1->intvalue );
			if ( v2 ) {
		...
		{
			Log_Write( "operator %s, value1 = %f", PunctuationFromNum( source->scriptstack, o->m_operator ), v1->floatvalue );
			if ( v2 ) {
		...
		lastoperatortype = o->m_operator;
		//if not an operator with arity 1
		if ( o->m_operator != P_LOGIC_NOT
			 && o->m_operator != P_BIN_NOT ) {
			//remove the second value if not question mark operator
			if ( o->m_operator != P_QUESTIONMARK ) {
				v = v->next;
			}
			//
			if ( v->prev ) {
