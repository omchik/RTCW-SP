...
static bool initFlameChunks = qfalse;
...
static bool initFlameShaders = qtrue;
...
void CG_UpdateFlamethrowerSounds( void ) {
	...
	for ( cent = cg_entities, i = 0; i < cgs.maxclients; cent++, i++ ) {
		if ( centFlameInfo[i].lastDmgCheck > centFlameInfo[i].lastDmgUpdate &&
			...
			CG_ClientDamage( i, centFlameInfo[i].lastDmgEnemy, clientDamage_t::CLDMG_FLAMETHROWER );
