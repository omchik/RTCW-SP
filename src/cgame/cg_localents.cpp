...
void CG_FragmentBounceSound( localEntity_t *le, trace_t *trace ) {
	...
	} else if ( le->leBounceSoundType == leBounceSoundType_t::LEBS_BRASS ) {

//----(SA) added
	} else if ( le->leBounceSoundType == leBounceSoundType_t::LEBS_ROCK ) {
...
//----(SA) end
	...
	} else if ( le->leBounceSoundType == leBounceSoundType_t::LEBS_BONE ) {
	...
	if ( le->leFlags & LEF_PLAYER_DAMAGE ) {
		...
		if ( newOrigin[2] < le->refEntity.origin[2] ) {
			...
			if ( i == 3 ) {
				...
				CG_ClientDamage( cg.snap->ps.clientNum, ENTITYNUM_WORLD, clientDamage_t::CLDMG_DEBRIS );
	...
	if ( le->breakCount ) {
		...
		for ( i = 0; i <= le->breakCount; i++ ) {
			...
			if ( nle->sizeScale < 0.7 ) {
				nle->sizeScale = 0.7;
				nle->leBounceSoundType = leBounceSoundType_t::LEBS_NONE;
			}
			// move us a bit
	...
	while ( time <= cg.time ) {
		...
		if ( alpha > 0.5 && ( le->lastSpiritDmgTime < time - 100 ) && enemyDist < 24 ) {
			...
			if ( !( le->ownerNum != cg.snap->ps.clientNum ? cg_entities[le->ownerNum].currentState.eFlags & EF_DEAD : cg.snap->ps.pm_type == PM_DEAD ) ) {
				// inflict the damage!
				CG_ClientDamage( cg_entities[le->ownerNum].currentState.otherEntityNum2, le->ownerNum, clientDamage_t::CLDMG_SPIRIT );
