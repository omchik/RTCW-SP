...
void CG_LaunchGib( centity_t *cent, vec3_t origin, vec3_t angles, vec3_t velocity, qhandle_t hModel, float sizeScale, int breakCount ) {
	...
	case AICHAR_ZOMBIE:
		...
		le->leBounceSoundType = leBounceSoundType_t::LEBS_BONE;
	...
	if ( cent->currentState.aiChar == AICHAR_HELGA || cent->currentState.aiChar == static_cast<int>(AICHAR_HEINRICH)) {
		// bit bouncier
		//le->pos.trType = TR_GRAVITY_LOW;
		le->bounceFactor = 0.4;
