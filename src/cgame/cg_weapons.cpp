...
static void CG_CalculateWeaponPosition( vec3_t origin, vec3_t angles ) {
	...
	if ( cg.predictedPlayerState.leanf != 0 ) {
		...
		angles[PITCH] += ( std::abs( cg.predictedPlayerState.leanf ) / 2.0f );
...
*/                                                                                                                              //
		...
		for ( i = 0; i < MAX_TESLA_BOLTS; i++ ) {
			if ( numSorted && i / numSorted < 1 /*(MAX_TESLA_BOLTS/3)*/ ) {  // bolts per enemy
				...
				// apply damage
				CG_ClientDamage( visEnemiesSorted[j], cent->currentState.number, clientDamage_t::CLDMG_TESLA );
				// show the effect
...
=============
*/
static bool debuggingweapon = qfalse;
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent ) {
	...
	weapSelect = static_cast<weapon_t>(cg.weaponSelect);
...
void CG_NextWeap( bool switchBanks ) {
	...
	int num, curweap;
	bool nextbank = qfalse;     // need to switch to the next bank of weapons?
	int i, j;
...
void CG_PrevWeap( bool switchBanks ) {
	int bank = 0, cycle = 0, newbank = 0, newcycle = 0;
	int num, curweap;
	bool prevbank = qfalse;     // need to switch to the next bank of weapons?
	int i, j;
...
void CG_ClientDamage( int entnum, int enemynum, int id ) {
	if ( id > clientDamage_t::CLDMG_MAX ) {
