...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	if ( cg_showAIState.integer && cent->currentState.aiChar ) {
		CG_PlayerFloatSprite( cent, cgs.media.aiStateShaders[static_cast<int>(cent->currentState.aiState)], 48 );
		return;
	}
...
void CG_AddZombieFlameEffect( centity_t *cent ) {
	...
	*/
	...
	// bool active=qfalse; // TTimo: unused
...
void CG_Player( centity_t *cent ) {
	...
	int clientNum;
	int renderfx;
	bool shadow;       //, drawweap = qtrue; // TTimo: unused
	float shadowPlane;
...
//----(SA)	modified
				...
				cent->currentState.aiChar == static_cast<int>(AICHAR_HEINRICH)) {
		...
		} else if ( cent->currentState.aiChar == static_cast<int>(AICHAR_HEINRICH)) {
			tags = &heinrichTags[0];
