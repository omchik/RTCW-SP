...
bool g_waitingForKey = qfalse;
bool g_editingField = qfalse;
...
void Menu_TransitionItemByName( menuDef_t *menu, const char *p, rectDef_t rectFrom, rectDef_t rectTo, int time, float amt ) {
	...
	for ( i = 0; i < count; i++ ) {
		...
		if ( item != NULL ) {
			...
			item->window.rectEffects2.y = std::abs( rectTo.y - rectFrom.y ) / amt;
			item->window.rectEffects2.w = std::abs( rectTo.w - rectFrom.w ) / amt;
			item->window.rectEffects2.h = std::abs( rectTo.h - rectFrom.h ) / amt;
...
//		} else if (key == K_TAB || key == K_UPARROW || key == K_DOWNARROW) {
	...
	if ( down && !( menu->window.flags & WINDOW_POPUP ) && !Rect_ContainsPoint( &menu->window.rect, DC->cursorx, DC->cursory ) ) {
		static bool inHandleKey = qfalse;
		// TTimo: gcc: suggest parentheses around && within ||
		// initial line:
