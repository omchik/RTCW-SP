...
static int PM_FootstepForSurface( void ) {
	...
	if ( pm->ps->aiChar == static_cast<int>(AICHAR_HEINRICH) ) {
		return EV_FOOTSTEP;
	}
...
//----(SA)	removed 'quickgrenade'
	...
	if ( pm->cmd.buttons & BUTTON_USE_HOLDABLE ) {
		if ( !( pm->ps->pm_flags & PMF_USE_ITEM_HELD ) ) {
			...
			if ( pm->cmd.holdable ) {
				item = BG_FindItemForHoldable( static_cast<holdable_t>(pm->cmd.holdable) );
...
if ( pm->ps->weapon ) {
	int ammoAvailable;
	bool reloadingW, playswitchsound = qtrue;

	ammoAvailable = PM_WeaponAmmoAvailable( pm->ps->weapon );

	if ( ammoNeeded > ammoAvailable ) {

		reloadingW = (bool)( ammoNeeded <= pm->ps->ammo[ BG_FindAmmoForWeapon( pm->ps->weapon )] ); // you have ammo for this, just not in the clip
