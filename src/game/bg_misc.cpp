...
gitem_t *BG_FindItemForWeapon( weapon_t weapon ) {
	...
	static bool lookupTableInit = qtrue;
...
weapon_t BG_FindClipForWeapon( int weapon ) {
	...
	if ( lookupTableInit ) {
		for ( i = 0; i < NUM_TABLE_ELEMENTS; i++ ) {
			...
			for ( it = bg_itemlist + 1 ; it->classname ; it++ ) {
				if ( it->giType == IT_WEAPON && it->giTag == i ) {
					lookupTable[i] = static_cast<weapon_t>(it->giClipIndex);
				}
			}
		}
		// table is created
...
weapon_t BG_FindAmmoForWeapon( int weapon ) {
	...
	if ( lookupTableInit ) {
		for ( i = 0; i < NUM_TABLE_ELEMENTS; i++ ) {
			...
			for ( it = bg_itemlist + 1 ; it->classname ; it++ ) {
				if ( it->giType == IT_WEAPON && it->giTag == i ) {
					lookupTable[i] = static_cast<weapon_t>(it->giAmmoIndex);
				}
			}
		}
		// table is created
