...
bool weaponStringsInited = qfalse;
...
//----(SA) // RF mod
				...
				if ( parseMode == animScriptParseMode_t::PARSEMODE_ANIMATION ) {
					currentScript = &modelInfo->scriptAnims[indexes[0]][indexes[1]];
					parseMovetype = static_cast<scriptAnimMoveTypes_t>(indexes[1]);
				} else if ( parseMode == animScriptParseMode_t::PARSEMODE_CANNED_ANIMATIONS ) {
		...
		case animScriptParseMode_t::PARSEMODE_EVENTS:
			...
			if ( !Q_stricmp( token, "{" ) ) {
				...
				if ( parseMode == animScriptParseMode_t::PARSEMODE_STATECHANGES ) {
...
int BG_ExecuteCommand( playerState_t *ps, animScriptCommand_t *scriptCommand, bool setTimer, bool isContinue, bool force ) {
	...
	if ( scriptCommand->bodyPart[0] ) {
		...
		if ( scriptCommand->bodyPart[0] == ANIM_BP_BOTH || scriptCommand->bodyPart[0] == ANIM_BP_LEGS ) {
			playedLegsAnim = ( BG_PlayAnim( ps, scriptCommand->animIndex[0], static_cast<animBodyPart_t>(scriptCommand->bodyPart[0]), duration, setTimer, isContinue, force ) > -1 );
		} else {
			BG_PlayAnim( ps, scriptCommand->animIndex[0], static_cast<animBodyPart_t>(scriptCommand->bodyPart[0]), duration, setTimer, isContinue, force );
		}
	}
	if ( scriptCommand->bodyPart[1] ) {
		...
		if ( scriptCommand->bodyPart[1] == ANIM_BP_BOTH || scriptCommand->bodyPart[1] == ANIM_BP_LEGS ) {
			playedLegsAnim = ( BG_PlayAnim( ps, scriptCommand->animIndex[1], static_cast<animBodyPart_t>(scriptCommand->bodyPart[1]), duration, setTimer, isContinue, force ) > -1 );
		} else {
			BG_PlayAnim( ps, scriptCommand->animIndex[1], static_cast<animBodyPart_t>(scriptCommand->bodyPart[1]), duration, setTimer, isContinue, force );
		}
	}
